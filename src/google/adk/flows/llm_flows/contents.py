...
def _is_part_invisible(p: types.Part) -> bool:
    ...
    return getattr(p, "thought", False) or not (
        p.text or p.inline_data or p.file_data or p.function_call or p.function_response
    )
...
def _is_event_belongs_to_branch(invocation_branch: Optional[str], event: Event) -> bool:
